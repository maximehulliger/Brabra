...
public class Cylinder extends ProMaster {
	...
	public static void initCylinder() {
		...
		for(int i = 0; i < x.length; i++) {
		  angle = (TangibleGame.TWO_PI / cylinderResolution) * i;
		  x[i] = TangibleGame.sin(angle) * cylinderRadius;
		  z[i] = TangibleGame.cos(angle) * cylinderRadius;
		}
		
		//le tube
		PShape openCylinder = app.createShape();
		openCylinder.beginShape(TangibleGame.QUAD_STRIP);
		for(int i = 0; i < x.length; i++) {
		...
		topCylinder.beginShape(TangibleGame.TRIANGLE_FAN);
		...
		cylinder = app.createShape(TangibleGame.GROUP);
