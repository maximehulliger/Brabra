...
public class Menu extends Interface {
	//title & buttons
	private static final int titleOffsetX = Brabra.width/2+250, titleOffsetY = 100; 
	...
	private static final int buttonOffsetX = Brabra.width/2 - buttonSizeX - 100,
				buttonOffsetY = Brabra.height/2 - ((nbButton-1)*buttonDiffY + buttonSizeY)/2;
	//help: button, zone & text
	private static final int helpButtonOffsetX = Brabra.width/2+150, helpButtonOffsetY = 200;
	...
	public void mouseReleased() {
		...
		for (int i=0; i<nbButton; i++, y+=buttonDiffY) {
			...
			if (diffx > 0 && diffx < buttonSizeX && diffy > 0 && diffy < buttonSizeY) {
				switch (i) {
				case 0:
					app.setView(Brabra.View.RealGame);
					return;
				case 1:
					app.setView(Brabra.View.TrivialGame);
					return;
				case 2:
					app.setView(Brabra.View.Calibration);
					return;
				default:
					return;
				}
